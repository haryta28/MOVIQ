"""
Full DB reset + clean reseed script.
Run once from backend/ directory:  python reset_and_seed.py
"""
import asyncio
import os
...
client = motor.motor_asyncio.AsyncIOMotorClient(
    MONGO_URL,
    tlsAllowInvalidCertificates=True,  # Mac Python 3.13 SSL cert fix
)
db     = client[DB_NAME]
...
# ─── USERS: 3 Admins + 3 Agency heads (all password = demo1234) ────────────────
USERS = [
    # ── ADMINS ──────────────────────────────────────────────────────────────────
    {
    ...
    # ── AGENCY HEADS (linked to agencies a1, a2, a3) ───────────────────────────
    {
...
COLLECTIONS_TO_RESET = [
    ...
    ("field_executives", FIELD_EXECUTIVES),
...
# Collections to wipe but NOT reseed (live data)
COLLECTIONS_TO_WIPE_ONLY = [
    "whatsapp_sessions",
    "vehicle_submissions",
]
...
async def main():
    print("🔄 Starting full DB reset...")
    
    for coll_name, docs in COLLECTIONS_TO_RESET:
        await db[coll_name].drop()
        if docs:
            await db[coll_name].insert_many([{**d} for d in docs])
        print(f"  ✅ {coll_name}: wiped & reseeded ({len(docs)} docs)")
    
    for coll_name in COLLECTIONS_TO_WIPE_ONLY:
        await db[coll_name].drop()
        print(f"  🗑️  {coll_name}: wiped (live data cleared)")
    ...
    print("\n  ADMINS")
    print("  -------")
    print("  admin@moviq.in          →  Deepak Bansal  (Super Admin)")
    print("  priya@moviq.in          →  Priya Sharma   (Platform Admin)")
    print("  rohan@moviq.in          →  Rohan Gupta    (Operations Admin)")
    print("\n  AGENCY USERS")
    print("  ------------")
    print("  saurav@brightads.in     →  Saurav Mehta   (BrightAds Media)")
    print("  anita@metroout.in       →  Anita Sharma   (Metro Outdoor)")
    print("  rahul@urbanreach.co     →  Rahul Kapoor   (UrbanReach)")
