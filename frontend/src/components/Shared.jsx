...
// ── KpiCard ────────────────────────────────────────────────────────────────────
// React.memo prevents re-renders when parent updates (e.g. 30s notification poll).
export const KpiCard = memo(function KpiCard({
  label, value, icon: Icon, delta, deltaLabel = 'vs last month', accent = 'blue',
}) {
  const accentMap = {
...
// ── ProgressBar ────────────────────────────────────────────────────────────────
export const ProgressBar = memo(function ProgressBar({ value, max = 100, color = 'bg-red-600' }) {
...
// ── PageHeader ─────────────────────────────────────────────────────────────────
export const PageHeader = memo(function PageHeader({ title, description, actions }) {
  return (
...
// ── MiniBarChart ───────────────────────────────────────────────────────────────
export const MiniBarChart = memo(function MiniBarChart({ data, valueKey, labelKey, color = 'bg-red-500' }) {
  // useMemo prevents recomputing max on every render when parent re-renders
  const max = useMemo(() => Math.max(...data.map(d => d[valueKey])), [data, valueKey]);
  return (
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="flex-1 h-full flex flex-col items-center justify-end gap-2">
            <div
              className={`w-full ${color} rounded-md transition-all hover:opacity-90 group relative cursor-pointer`}
              style={{ height: `${h}%` }}
            >
