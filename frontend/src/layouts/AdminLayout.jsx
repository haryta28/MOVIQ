...
import { MapPin, LayoutDashboard, Building2, Megaphone, Users, ShieldAlert, BarChart3, CreditCard, Layers, LogOut, Bell, Search } from 'lucide-react';
          ...
          {nav.map(item => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition ${isActive ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`
                }
              >
        ...
        <div className="border-t border-slate-200 p-3">
          <div className="flex items-center gap-3 rounded-md p-2">
            <Avatar className="h-9 w-9">
              <AvatarFallback className="bg-blue-600 text-white text-xs">{user?.avatar || 'A'}</AvatarFallback>
            </Avatar>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center px-6 gap-4 sticky top-0 z-10">
          <div className="relative flex-1 max-w-md">
            ...
            <Input placeholder="Search agencies, campaigns, tasks..." className="pl-9 bg-slate-50 border-slate-200" />
          </div>
