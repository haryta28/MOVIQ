...
import { MapPin, LayoutDashboard, Megaphone, ListChecks, Users, Images, FileBarChart, Map, LogOut, Bell, Search, Sparkles } from 'lucide-react';
    ...
    <div className="min-h-screen flex bg-slate-50">
      <aside className="w-64 shrink-0 bg-white border-r border-slate-200 flex flex-col">
        <div className="h-16 flex items-center gap-2 px-5 border-b border-slate-200">
          ...
          <div>
            <div className="font-bold text-slate-900 leading-tight">gOGig</div>
          ...
          {nav.map(item => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition ${isActive ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`
                }
              >
        ...
        <div className="mx-3 mb-3 p-3 rounded-lg border border-indigo-100 bg-gradient-to-br from-indigo-50 to-white">
          <div className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-indigo-600" />
            <div className="text-sm font-semibold text-slate-800">Upgrade plan</div>
          </div>
          <p className="text-xs text-slate-500 mt-1">Unlock unlimited campaigns & white-label reports.</p>
          <Button size="sm" className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 text-white h-8">Upgrade</Button>
        </div>

        <div className="border-t border-slate-200 p-3">
          <div className="flex items-center gap-3 rounded-md p-2">
            <Avatar className="h-9 w-9">
              <AvatarFallback className="bg-indigo-600 text-white text-xs">{user?.avatar || 'A'}</AvatarFallback>
            </Avatar>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center px-6 gap-4 sticky top-0 z-10">
          <div className="relative flex-1 max-w-md">
            ...
            <Input placeholder="Search campaigns, tasks, executives..." className="pl-9 bg-slate-50 border-slate-200" />
          </div>
          <div className="flex items-center gap-2">
            <DropdownMenu>
